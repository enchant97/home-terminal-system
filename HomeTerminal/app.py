...
def api_get_hw():
    ...
    if Api_Key.query.filter_by(key=api_key).first():
...
def create_api_key():
    ...
    if username:
        try:
            if Api_Key.query.filter_by(owner=username).first():
                flash(f"You already have a api key for '{username}'")
            else:
                new_key = Api_Key(key=hash_str(str(datetime.utcnow())), owner=username)
...
def userhome():
    ...
    if username:
        ...
        return render_template("/terminal_page.html", username=session.get("username"), messages=Message.query.filter_by(removed=0).all(), notifications=notifications)
    else:
        return redirect("/")
...
def newaccount():
    if session.get("username", None) == "terminal" or not User.query.first():
        try:
            if request.method == "POST":
...
@app.route("/logout")
def do_logout():
    loggedin = session.pop("username", False)
    if loggedin:
        logging.debug(f"{loggedin} logged out")
        flash("logged out")
    return redirect("/")
...
def command_center():
    username = session.get("username", False)
    if username:
        if username == CONFIG.get_admin_username():
...
def get_servercontrol():
    if session.get("username", False) == CONFIG.get_admin_username():
...
def get_pd1_subloc():
    # TODO: return json list with all sublocations
    if session.get("username", False):
