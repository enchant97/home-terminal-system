...
class SubLocation(Base):
    ...
    main_loc_id = db.Column(db.Integer, db.ForeignKey(
        "pm_mainlocs.id"), nullable=False)
    lat = db.Column(
        db.Float(precision=10, decimal_return_scale=None), nullable=False)
    lng = db.Column(
        db.Float(precision=10, decimal_return_scale=None), nullable=False)
...
class FullEvent(Base):
    ...
    subloc_id = db.Column(db.Integer, db.ForeignKey(
        "pm_sublocs.id"), nullable=False)
...
class UserEvent(Base):
    ...
    full_event_id = db.Column(db.Integer, db.ForeignKey(
        "pm_fullevents.id"), nullable=False)
